...
async function ensureUniqueSlug(
  ...
  while (attempt < 200) {
    ...
    if (error) {
      throw new Error(`Unable to ensure unique slug for ${table}: ${error.message}`);
    }
  ...
  throw new Error(`Unable to generate unique slug for ${table} from "${baseSlug}".`);
}
...
export async function POST(request: Request) {
  const parsed = contentPackageSchema.safeParse(await request.json().catch(() => null));
  ...
  if (parsed.data.action === "save_draft") {
    ...
    const normalizedExistingTopicSlugs = dedupeKeepOrder(
      parsed.data.data.existingTopicSlugs.map((slug) => slug.trim().toLowerCase()),
    );
    ...
    if (normalizedExistingTopicSlugs.length) {
      ...
      const bySlug = new Map(
        (((existingTopicRows as TopicRow[] | null) ?? []).map((row) => [row.slug, row])),
      );
    ...
    const { data: duplicateQuestionRows, error: duplicateQuestionError } = await supabase
      .from("questions")
  ...
  const { data: duplicatePublishedRows, error: duplicatePublishedError } = await supabase
    .from("questions")
    .select("slug, title, status")
    .eq("status", "published")
  ...
  const topicIds = dedupeKeepOrder(
    ((questionTopicRows ?? []).map((row) => row.topic_id).filter(Boolean) as string[]),
  );
